...
//
// ---------------------------------------------------------------------------
// This file is meant to be copied, and this is a copy.
//
// Taken from agent/public/three-act.js, which says so in its own header: the
// judgements belong wherever somebody writes a run, and sharing the rendering
// instead would mean sharing a design system. ClaireTv's console draws the
// findings as a row of lines; the agent's desk draws them as dots. Same
// function, same verdicts.
//
// ONE CHANGE FROM THE ORIGINAL, and it is a change of question rather than of
// rule: a micro-drama releases ten of sixty, so the shape being checked is the
// shape of what is out, not of the whole arc. checkShape is unchanged and
// still answers about the episodes it is handed — the console simply hands it
// the released ones and says so on the label.
//
 ...
 *  @param {number} [planned] total episodes the run will have, when more are
 *         coming than are written. Omit it and this behaves exactly as before.
 *  @returns {{ n:number, perAct:number[], checks:{ok:boolean, text:string}[] }} */
export function checkShape(episodes, planned = 0) {
  ...
  /* A RELEASED RUN IS NOT A SHORT RUN, and judging one as the other is how a
     correct act one gets told it has no ending. A micro-drama puts out ten of
     sixty: positions are measured against the sixty, and the beats that have
     not been reached yet are reported as not yet rather than as missing.
     Omit `planned` and every judgement below is the original one. */
  const total = planned > n ? planned : n;
  ...
  const at = (fn) => {
    ...
    return i < 0 ? null : (i + 1) / total;
  };
  const pct = (v) => Math.round(v * 100) + "%";
  const checks = [];
  const add = (ok, text) => checks.push({ ok, text });

  const third = perAct[2] / total;
  if (partial && !perAct[2]) add(true, `Act three not released yet — ${n} of ${total} out`);
  else add(third >= 0.18, third >= 0.18
    ? `Act three is ${pct(third)} of the run`
    : `Act three is only ${pct(third)} — the ending has no room`);

  const mid = at("midpoint");
  add(mid !== null ? Math.abs(mid - 0.5) <= 0.12 : partial,
    mid === null ? (partial ? "Midpoint not reached yet" : "No midpoint — nothing turns")
  ...
  add(at("all is lost") !== null || partial,
    at("all is lost") !== null ? "Act two has a bottom"
      : partial ? "All is lost not reached yet"
      : "No all is lost — act two never bottoms out");

  add(at("climax") !== null || partial,
    at("climax") !== null ? "There is a climax"
      : partial ? "Climax not reached yet" : "No climax");
