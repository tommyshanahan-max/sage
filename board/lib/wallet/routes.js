...
export function createWalletRouter({ service, passkeys, provider, identify, testConfirm = false }) {
  ...
  /* ---- reading ---- */
  r.get("/api/wallet", run(async (me) => ({ ...(await service.state(me)), testConfirm: allowTest, me: { id: me.id, handle: me.handle } })));
