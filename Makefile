...
weidian-reviews: ## Old 微店 reviews onto the matching products: make weidian-reviews SHOP=https://weidian.com/s/1202970134 [MAX=40] [DRY=1] [ADD=1] [PHOTOS=1] [NAMES=1]
	...
	@$(COMPOSE) --profile post run --rm --no-deps -T -v "$(CURDIR)/scripts:/app/seed:ro" post-browser \
	  ...
	  --file /data/weidian.json $(if $(DRY),--dry,) $(if $(ADD),--add,) $(if $(PHOTOS),--photos,) $(if $(NAMES),--names,)
...
shop-contact: ## How her buyers reach her: make shop-contact WHO="Mei" WECHAT="meimei_au" [QR=url] [WEIDIAN=url] [SINCE=2017] [AI=1]
	...
	$(COMPOSE) run --rm --no-deps -T -v "$(CURDIR)/scripts:/seed:ro" --entrypoint node board \
	  ...
	  $(if $(WEIDIAN),--weidian "$(WEIDIAN)",) $(if $(SINCE),--since "$(SINCE)",) $(if $(AI),--ai "$(AI)",)
