...
const MAKE = rest.includes("--make");
if (!base || !key || !A || !B) {
  console.error('usage: pair.mjs <board url> <admin key> --a "Name" --b "Name" [--make]');
  process.exit(2);
}
...
const r = MAKE
  ? await fetch(base + "/api/pair",
      { method: "POST", headers: head, body: JSON.stringify({ a: A, b: B }) })
  : await fetch(base + "/api/pair?a=" + encodeURIComponent(A) + "&b=" + encodeURIComponent(B),
      { headers: head });
...
if (!r.ok) {
  ...
  } else if (d.error === "draft") {
    console.error("");
    console.error("  No page yet: " + (d.who || []).join(", "));
    console.error("  A follow points at a page. Until they have made one there is");
    console.error("  nothing to point at \u2014 let them finish it, then run this again.");
    console.error("");
  } else if (d.error === "self") {
    console.error("That is the same person twice.");
...
console.log("");
if (MAKE) {
  /* SAID OUT LOUD, both when it wrote and when it did not. Running this twice
     is not an error and must not look like one — but "nothing to do" and
     "done" are different facts and the operator is owed which. */
  console.log((d.made || []).length
    ? "  Followed: " + d.made.join("   and   ")
    : "  They already followed each other. Nothing to write.");
  console.log("");
}
...
line("both follow each other — " + d.b.name + " → " + d.a.name,
  d.bFollowsA, d.b.name + " taps it themselves — or write it for them with:  make match A=\""
    + d.a.name + "\" B=\"" + d.b.name + "\"");
...
line("something in common: " + (d.shared.length ? d.shared.join(", ") : "nothing"),
  d.shared.length > 0, "Their rooms do not answer each other. Buying answers selling, raising answers investing, looking for work answers hiring. Two people both selling are not a pair."
    + (d.tick ? "\n      " + d.a.name + " ticking \"" + d.tick + "\" on their own profile would pair them. It is their box, and this never ticks it for them — a sentence somebody did not write is not theirs." : ""));

console.log("");
/* SHUT IS NOT ONE OF THE THREE TESTS and it beats all of them — see
   threadState. Printed only when it is true, because a line saying "nobody has
   blocked anybody" on every healthy pair is noise. */
if (d.shut) {
  console.log("One of them has blocked or left the other. That closes the thread");
  console.log("whatever the four lines above say, and it is theirs to undo.");
  console.log("");
}
console.log(d.matched && !d.shut
